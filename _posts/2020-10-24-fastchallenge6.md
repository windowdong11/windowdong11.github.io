...
0. BST에 최상위 노드가 없는 경우 최상위 노드에 데이터를 삽입하고 종료한다.
1. 최상위 노드부터 탐색을 시작한다.  
2. 현재 노드의 값보다 삽입하려는 값이 작다면 왼쪽 노드부터 다시 탐색을 시작하고,  
현재 노드의 값보다 삽입하려는 값이 크다면 오른쪽 노드부터 다시 탐색을 시작한다.
3. 다음 탐색위치에 노드가 없을때까지 2번을 반복한다.
4. 다음 탐색위치에 데이터를 삽입한다.
...
1. 삭제하고자 하는 노드에 자식이 없는 경우(Leaf Node)  
...
2. 삭제하고자 하는 노드의 자식이 하나만 존재하는 경우  
...
3. 삭제하고자 하는 노드의 자식이 둘다 존재하는 경우(가장복잡)  
